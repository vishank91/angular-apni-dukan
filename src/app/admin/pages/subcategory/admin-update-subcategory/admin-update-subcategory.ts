...
@Component({
  selector: 'app-admin-update-subcategory',
  standalone: false,
  templateUrl: './admin-update-subcategory.html',
  styleUrl: './admin-update-subcategory.css',
})
export class AdminUpdateSubcategory {
  id = ""
  ...
  constructor(private dataService: DataService, private router: Router, private activatedRoute: ActivatedRoute) {
    this.activatedRoute.queryParams.subscribe((params: any) => {
      if (params.id) {
        this.id = params.id
        this.dataService.getSingleSubcategory(this.id).subscribe((response: any) => {
  ...
  getInputFile(e: any) {
    ...
    if (!(file.type === "image/jpeg" || file.type === "image/jpg" || file.type === "image/png" || file.type === "image/webp" || file.type === "image/gif"))
      ...
      this.myForm.patchValue({ pic: `subcategory/${file.name}` })
  }
  setErrorMessage() {
    this.nameError = ""
  }
  ...
  postData() {
    this.dataService.getSubcategory().subscribe((response: any) => {
      ...
      if (item)
        this.nameError = "Subcategory With This Name is Already Exist"
      else {
        item = {
          id: this.id,
        ...
        this.dataService.updateSubcategory(item).subscribe((response: any) => {
